...
export default function Example3Card({ index, isActive, onCompleted, pageWidth }: Props) {
  ...
  const goalTextSize = isSmall ? 14 : 16;
  const inputTextSize = isSmall ? 14 : 15;
  const stackLimeSize = isSmall ? 26 : 30;
  const rowHeight = isSmall ? 38 : isNarrowHeight ? 38 : 42;
  const rowGap = isNarrowHeight ? 5 : 6;
  ...
  const rows = [
    { num: '01', text: '30 min French lesson', style: rowStyle(row1), checkStyle: checkAnim(chk1) },
    { num: '02', text: '30 min French podcast', style: rowStyle(row2), checkStyle: checkAnim(chk2) },
    { num: '03', text: '15 min speaking practice', style: rowStyle(row3), checkStyle: checkAnim(chk3) },
    { num: '04', text: '10 new words', style: rowStyle(row4), checkStyle: checkAnim(chk4) },
  ];
    ...
    <View style={[styles.container, { width: pageWidth }]}>
      <View style={styles.exampleArea}>
        ...
        <Animated.View style={[styles.goalCard, goalStyle]}>
          ...
          <Text
            style={[styles.goalText, { fontSize: goalTextSize, color: LIME }]}
            numberOfLines={2}
